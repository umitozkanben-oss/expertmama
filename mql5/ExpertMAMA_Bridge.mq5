...
datetime g_last_bars[12][4]; // [symbol][tf]

int OnInit(void)
  {
   // last bar zamanlarını sıfırla
   for(int i=0;i<12;i++) for(int j=0;j<4;j++) g_last_bars[i][j]=0;
   EventSetTimer(3);
   printf("ExpertMAMA Bridge v2 başladı → "+VPS_URL);
   printf("Takip edilen semboller: "+IntegerToString(SYM_COUNT));
...
void OnTick(void)
  {
   g_tick++; g_pos++;
   // Her sembolün tick'ini gönder
   if(g_tick >= TickEvery)
     {
      for(int i=0;i<SYM_COUNT;i++) SendTick(SYMBOLS[i]);
      g_tick=0;
     }
   if(g_pos >= PosEvery)
     {
   ...
   // Yeni bar kontrolü — her sembol, her TF
   for(int i=0;i<SYM_COUNT;i++)
     for(int j=0;j<TF_COUNT;j++)
       {
        datetime b=iTime(SYMBOLS[i],TFS[j],0);
        if(g_last_bars[i][j]!=0 && b!=g_last_bars[i][j])
           SendBars(SYMBOLS[i],TFS[j],TF_NAMES[j],3);
        g_last_bars[i][j]=b;
       }
  }
   ...
   if(!g_bars_sent)
     {
      ...
      SendPositions();
      g_bars_sent=true;
      printf("İlk bar verisi tamamlandı.");
     }
  }
...
void SendTick(string sym)
  {
   ...
   if(bid==0) return; // sembol yüklenmemiş
   string json="{";
   json+="\"symbol\":\""+sym+"\",";
